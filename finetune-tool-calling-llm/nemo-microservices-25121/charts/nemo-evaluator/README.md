...
| livenessProbeInternal.httpGet | object | `{"path":"/health","port":"http-internal"}` | Configures the liveness probe for the NeMo Evaluator internal microservice. The liveness probe checks if the container is running. The probe sends an HTTP GET request to the `/health` endpoint on the container's `http-internal` port. |
...
| readinessProbeInternal.httpGet | object | `{"path":"/health","port":"http-internal"}` | Configures the readiness probe for the NeMo Evaluator internal microservice. The readiness probe checks if the container is ready to receive traffic. The probe sends an HTTP GET request to the `/health` endpoint on the container's `http-internal` port. |
